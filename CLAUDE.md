...
# Testing (NUnit) — структура из 4 проектов
dotnet test                                              # Все тесты
dotnet test Tests/Tests.Unit                             # Unit-тесты
dotnet test Tests/Tests.Integration                      # Интеграционные тесты
dotnet test Tests/Tests.E2E                              # End-to-end тесты
...
├── winprutil/                 # Git submodule: Windows печать
├── Tests/                     # NUnit тесты (4 проекта)
│   ├── Tests.Unit/            # Модульные тесты (контроллеры, сервисы, модели)
│   ├── Tests.Integration/     # Интеграционные тесты (КМХ, Common, документы)
...
Тесты разбиты на 4 проекта по типу:

| Проект | Назначение | Ключевые тесты |
|--------|------------|----------------|
| Tests.Unit | Модульные тесты | Controllers, Services, Models, DTOs |
| Tests.Integration | Интеграционные | КМХ (~168), Common, AppConfigService |
| Tests.E2E | End-to-end | Placeholder (планируется) |
| Tests.Shared | Общие helpers | Fixtures, Test utilities |

```bash
# Запуск конкретных категорий тестов
dotnet test Tests/Tests.Unit                          # Unit-тесты
dotnet test Tests/Tests.Integration --filter "Namespace~KMH"  # КМХ тесты
dotnet test Tests/Tests.Integration --filter "Namespace~Common"  # Common тесты
```

**Статус тестов (v1.3.8):**
- ~315 работающих тестов (~48%), ~335 отключенных (~52%)
