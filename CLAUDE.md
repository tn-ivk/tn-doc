...
- **Улучшенная UX редактирования паспорта качества** (Ноябрь 2025):
  ...
  - ✅ **Оптимизирована ширина колонок таблицы показателей качества**:
    - Колонка "№" уменьшена с 40px до 30px для экономии пространства
    - Колонка "Наименование показателя" установлена на фиксированную ширину 400px
    - Улучшена читаемость таблицы за счёт большего пространства для названий параметров
  - ✅ **Исправлена UX проблема в StatusBar**: убрана ложная кликабельность индикаторов устройств и сервисов
    - Индикаторы больше не выглядят кликабельными (курсор остается стандартным)
    - Удален нереализованный обработчик клика для будущего функционала
    - Сохранены tooltip подсказки с информацией о состоянии устройств и сервисов
  - ✅ **Упрощён интерфейс комбобокса подписантов**: убрана dropdown иконка (шеврон) из поля ИОФ
    - Добавлен универсальный prop `hideDropdownIcon` в FormField.vue для переиспользования
    - Освобождено пространство справа (~32px)
    - Клик по всему полю раскрывает выпадающий список
  - ✅ **Унификация стилей выпадающих списков комбобоксов методов испытаний**:
    - Решена проблема стилизации overlay элементов PrimeVue, создаваемых вне компонента
    - Использован атрибут `panelClass="method-select-panel"` для таргетирования выпадающего списка
    - Добавлены глобальные стили для унификации с внешним видом комбобоксов подписантов
    - Компонент: `PassportMethodSelect.vue` с глобальными стилями `.method-select-panel`
    - Теперь все комбобоксы в редакторе паспорта качества имеют единообразный внешний вид
  - ✅ **Исправлено отображение выпадающих списков комбобоксов**: устранено обрезание границами таблицы
    - Изменён рендеринг выпадающего списка Select: `appendTo="body"` вместо `appendTo="self"`
    - Overlay теперь рендерится напрямую в `<body>`, минуя ограничение `overflow: auto` родительского контейнера
    - Добавлены глобальные стили для выпадающих списков (.p-select-overlay, .p-select-list, .p-select-option)
    - Удалены избыточные scoped стили, которые не работали при рендеринге в body
    - Сохранён оригинальный внешний вид и поведение (hover, focus, transitions)
    - Компонент: `FormField.vue` с двумя секциями стилей (scoped для компонента, глобальные для overlay)
  - ✅ **Обновлена иконка кнопок редактирования**: заменена pi-pencil на более современную pi-pen-to-square
    - Применено для кнопок редактирования метода испытаний и результата
    - Улучшена визуальная индикация функции редактирования
    - Компоненты: `PassportMethodSelect.vue`, `PassportMethodSelectWithHistory.vue`, `PassportResultCell.vue`, `PassportResultCellWithHistory.vue`
  - ✅ **Улучшена видимость иконок редактирования**: адаптивная стилизация под цвет фона поля
    - Светло-серая иконка (#E0E0E0) для белого фона полей
    - Тёмная иконка (#212121) для ELIS-заполненных полей с зелёным фоном
    - Синяя (#2f6fed) при hover, тёмно-синяя (#1e54d4) при active
    - Убран фон при hover/active для более чистого визуального стиля
  - ✅ **Оптимизировано центрирование значений в колонке Результат**: динамическая логика выравнивания
    - Автоматический расчёт padding в зависимости от количества иконок
    - Нет иконок: минимальный padding (8px), значение по центру
    - Одна иконка: средний padding (30px)
    - Две иконки: максимальный padding (62px)
    - Компонент: `PassportResultCell.vue`
  - ✅ **Улучшено позиционирование элементов управления**: оптимизированы отступы и расположение
    - Динамическое позиционирование кнопки редактирования результата (2px без индикатора, 30px с индикатором истории)
