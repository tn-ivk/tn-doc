...
## FastReport .NET Library Reference

FastReport — библиотека для генерации документов и отчётов. В проекте используется для создания PDF/HTML из шаблонов `.frx`.

### Базовое использование

```csharp
// Создание и запуск отчёта
Report report = new Report();
report.Load("reportfile.frx");           // Загрузка шаблона
report.RegisterData(dataSet);            // Регистрация данных
report.Prepare();                        // Подготовка отчёта
report.Show();                           // Показать отчёт
```

### Регистрация источников данных

```csharp
// DataSet (все таблицы, views и relations)
report.RegisterData(dataSet);
report.RegisterData(dataSet, "MyDataSet");  // с именем

// DataTable
report.RegisterData(dataSet.Tables["Orders"], "Orders");

// DataView
report.RegisterData(dataView, "MyView");

// Business Objects (IEnumerable)
report.RegisterData(myList, "Products", BOConverterFlags.AllProperties, maxNestingLevel: 3);

// После регистрации включить источник
report.GetDataSource("Products").Enabled = true;
```

### Создание отчёта программно

```csharp
Report report = new Report();
report.RegisterData(dataSet.Tables["Products"], "Products");
report.GetDataSource("Products").Enabled = true;

// Создание страницы A4
ReportPage page = new ReportPage();
page.Name = "Page1";
page.PaperWidth = 210;   // мм
page.PaperHeight = 297;  // мм
...
text.Text = "PRODUCTS";
text.Bounds = new RectangleF(0, 0, Units.Centimeters * 19, Units.Centimeters * 1);
text.HorzAlign = HorzAlign.Center;
text.Font = new Font("Tahoma", 14, FontStyle.Bold);
...
// Поле данных с выражением привязки
TextObject dataText = new TextObject();
dataText.Text = "[Products.ProductName]";
dataText.Bounds = new RectangleF(0, 0, Units.Centimeters * 10, Units.Centimeters * 0.5f);
dataBand.Objects.Add(dataText);

report.Prepare();
```

### Структура шаблона (.frx файл)
...
<Report ScriptLanguage="CSharp">
  ...
  <ReportPage Name="Page1">
    <ReportTitleBand Name="ReportTitle1" Height="75.6">
      <TextObject Name="Text1" Text="EMPLOYEES" HorzAlign="Center"/>
    </ReportTitleBand>
    <DataBand Name="Data1" DataSource="Employees">
      <TextObject Name="Text3" Text="[Employees.FirstName]"/>
...
html.EmbedPictures = true;   // Встроить картинки в HTML
html.SinglePage = true;      // Все страницы в один файл
html.SubFolder = false;      // Без поддиректории для ресурсов
html.Layers = true;          // Слойный HTML
html.Navigator = false;      // Без навигации
...
### Основные методы класса Report

| Метод | Описание |
|-------|----------|
| `Load(string fileName)` | Загрузить шаблон из файла |
| `FromFile(string fileName)` | Создать Report и загрузить шаблон |
| `FromStream(Stream stream)` | Загрузить из потока |
| `FromString(string utf8)` | Загрузить из строки |
| `RegisterData(...)` | Зарегистрировать источник данных |
| `GetDataSource(string alias)` | Получить источник данных |
| `Prepare()` | Подготовить отчёт к экспорту |
| `Export(ExportBase, string)` | Экспортировать в файл |
| `FindObject(string name)` | Найти объект по имени |
| `GetColumnValue(string name)` | Получить значение колонки |
| `SetParameterValue(name, value)` | Установить параметр |

### Типы бэндов (Bands)

| Band | Назначение |
|------|------------|
| `ReportTitleBand` | Заголовок отчёта (один раз в начале) |
| `PageHeaderBand` | Верхний колонтитул страницы |
| `DataBand` | Повторяется для каждой записи данных |
| `GroupHeaderBand` | Заголовок группы |
| `GroupFooterBand` | Подвал группы |
| `PageFooterBand` | Нижний колонтитул страницы |
| `ReportSummaryBand` | Итоги отчёта (один раз в конце) |

### Выражения в тексте

```
[DataSource.ColumnName]           — значение колонки
[Date]                            — текущая дата
[Page]                            — номер страницы
[TotalPages]                      — всего страниц
[[Expression].Method()]           — вызов метода
[IIF(condition, true, false)]     — условное выражение
```

### Использование в TN_Doc

В проекте FastReport используется через интерфейс `IDocumentEditor`:
- Метод `GetViewDoc(id)` возвращает JSON-данные для шаблона
- Метод `GetPathTemplateFile()` возвращает путь к `.frx` файлу
- Шаблоны хранятся в `TN_Doc/Doc/` с именованием `{Number}_{DocumentType}.frx`
- Конфигурация шаблонов в `TN_Doc/Cfg/Cfg{DocumentType}.json`
