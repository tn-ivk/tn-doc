...
3. **Множественные алиасы (массив)**
   - [ ] Проверить поле с `elisAlias: ["density", "Density20"]`
   - [ ] Убедиться, что используется первый найденный ключ из массива
   - [ ] Проверить, что fallback работает корректно (пробует все элементы массива по порядку)
...
**Примечание**: Кэширование оставлено для будущих версий, в первой версии не реализуется
...
#### 4.2 Документация для администраторов
...
#### 5.3 Тестирование на production-подобной среде
...
## ⚠️ Важное предупреждение по OPC интеграции

**РАБОТА С OPC - ЭТО ОТДЕЛЬНАЯ НЕЗАВИСИМАЯ ЗАДАЧА!**

- OPC интеграция при сохранении паспорта **НЕ СВЯЗАНА** с ELIS интеграцией
- Текущий механизм работы с OPC **РАБОТАЕТ КОРРЕКТНО**
- **ОСНОВНОЕ ТРЕБОВАНИЕ**: НЕ СЛОМАТЬ текущую реализацию работы с OPC
- OPC интеграция останется в старом коде (`TN_Doc/wwwroot/js/EditDoc.js:99-155`)
- Vue редактор будет использовать тот же механизм сохранения, что и старый HTML редактор
- Синхронизация с ИВК через OPC теги происходит независимо от источника данных (ELIS или ручной ввод)

**Файлы, отвечающие за OPC интеграцию** (НЕ трогать в рамках этой задачи):
- `TN_Doc/wwwroot/js/EditDoc.js` - запись тегов `ARM.ARM_FillActAndPassport`
- `TN.MessagingService` - OPC клиент для связи с ИВК
- Конфигурация OPC в `CfgApp.json` под "OpcConnectionSettings"
...
- **Этап 3**: 2-3 дня (оптимизация и улучшения)
- **Этап 4**: 1 день (документация)
- **Этап 5**: 2-3 дня (подготовка к production)

**Итого**: 9-13 рабочих дней

## Зависимости

- TN.ElisConnector (внешний сервис) - для получения данных из ELIS
- Vue Document Editor должен быть стабилен

**Примечание**: TN_MessagingService (OPC клиент) не является зависимостью этой задачи, работает независимо

## Критерии готовности (Definition of Done)

- [ ] Все чек-листы из Этапов 0-5 выполнены
- [ ] Документация обновлена
- [ ] Code review пройден
...
- **2025-11-07**: Убран Этап 3 (OPC интеграция)
  - OPC интеграция - это отдельная независимая задача, не связанная с ELIS
  - Текущий механизм OPC работает корректно и не требует изменений
  - Добавлено предупреждение: НЕ СЛОМАТЬ текущую реализацию OPC
  - Обновлена нумерация этапов (бывший Этап 4 → Этап 3, и т.д.)
  - Обновлён таймлайн: 9-13 рабочих дней (было 10-15)
- **2025-11-07**: Удалены задачи по доработке OPC
  - Убран критерий готовности о регресс-тестировании OPC
  - ELIS интеграция полностью независима от OPC
  - Предупреждения об OPC оставлены для информации
