...
## 🎯 Текущий статус: Этапы 0-1 завершены + КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ, готовность 90%
...
- [x] ⭐ **КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ElisAlias в IDocumentEditor** (2025-11-10)
  - [x] Добавлено свойство `ElisAlias` в класс `FormField` (IDocumentEditor.cs)
  - [x] Добавлено копирование `ElisAlias` в метод `BuildAdditionalInfoFields` (DocPassport.cs:1367)
  - [x] Пересобраны библиотеки: TN.DocGeneral → Passport → TN_Doc
  - [x] Коммиты: 5d2127e (TN.DocGeneral), 26c89a5 (Passport), 91b4cf6 (tn_doc)
  - [x] Теперь ElisAlias корректно передаётся из конфигурации на клиент
...
- **2025-11-10**: ⚠️ **КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ - ElisAlias в BuildAdditionalInfoFields**
  - **Проблема**: ElisAlias приходило как `null` вместо массива строк
  - **Причина**: Метод `BuildAdditionalInfoFields` не копировал `ElisAlias` из конфигурации
  - **Решение**: Добавлена строка `ElisAlias = item.ElisAlias?.ToList() ?? new List<string>()`
  - Изменения:
    * IDocumentEditor.cs: добавлено свойство `ElisAlias` в класс `FormField`
    * DocPassport.cs:1367: добавлено копирование `ElisAlias` из конфигурации
  - Пересобраны библиотеки: TN.DocGeneral → Passport → TN_Doc
  - Коммиты:
    * 5d2127e (TN.DocGeneral): добавлено свойство ElisAlias в FormField
    * 26c89a5 (Passport): исправлено заполнение ElisAlias в BuildAdditionalInfoFields
    * 91b4cf6 (tn_doc): обновлена ссылка на подмодуль tn.docgeneral
  - Статус готовности: 90% (был 85%)
  - **Теперь ElisAlias корректно передаётся из конфигурации на клиент!**
