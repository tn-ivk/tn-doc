...
## ✅ РЕАЛИЗОВАНО: Проверка наличия колонки DataARM

### Реализованные компоненты:

1. **Сервис проверки схемы БД** ✅:
   - `IDbSchemaCache` и `DbSchemaCache` в `TN_Doc/Models/Services/`
   - Кэширование результатов в `ConcurrentDictionary<(int deviceId, string tableName), bool>`
   - Проверка через `DBtService.GetTableInfo()` для определения наличия колонки `DataARM`

2. **API для проверки** ✅:
   - `HomeController.CanEditDocument(int idDevice, int idDoc)` - возвращает JSON с флагом `canEdit`
   - Логирование результатов проверки

3. **JavaScript интеграция** ✅:
   - `checkCanEditDocument()` - асинхронная функция проверки
   - Обновлена `GetDoc()` - проверка при загрузке документа
   - Обновлен обработчик смены типа документа
   - Обновлена `GetData()` - проверка при загрузке списка

4. **DI регистрация** ✅:
   - `IDbSchemaCache` зарегистрирован как `Scoped` в `Startup.cs`
   - Внедрен в `HomeController`

### Логика работы:
- Кнопка "Редактирование" показывается только если:
  - `isEditingDoc` = true (документ поддерживает редактирование)
  - `HasDataArm(deviceId, docType)` = true (колонка DataARM существует)
  - `isAllowEditAndSave` = true (пользователь имеет права)
- При отсутствии колонки `DataARM` кнопка редактирования скрывается
- Fail-safe подход: при ошибках проверки редактирование отключается