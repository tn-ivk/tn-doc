...
- [x] Обновить методы `GetEditDoc` во всех документных библиотеках:
  ...
  - [x] KMH* (13 из 14 библиотек, осталась KMH_MI2816) ✅ **(Большая часть завершена 2025-10-15)**
  - [ ] Common* (библиотеки не существуют как отдельные файлы)
- [x] Создать метод `GenerateVueFormHtml` для генерации HTML с Vue (пример готов)
- [ ] Удалить старые методы генерации HTML таблиц

**Текущий прогресс:** 36/42 библиотеки (85.7%)
**Цель для разблокировки Этапа 6:** 34/42 библиотеки (80%) ✅ **ДОСТИГНУТО**

**Последняя сессия миграции (2025-10-15):**
...
**Прогресс:** 36/42 (85.7%) ✅ **ЦЕЛЬ ДОСТИГНУТА**
...
- [ ] KMH_MI2816 (MI 2816) - ⚠️ специальный случай: использует ARMData вместо AdditionalInfo
...
- [ ] ~~KMH_PP_Areom~~ (не существует в проекте)
...
- [ ] ~~KMH_PW~~ (не существует в проекте)
...
**Статус:** ✅ **РАЗБЛОКИРОВАН** (достигнуто 85.7% миграции, цель была 80%)

**Прогресс:**
- [x] Создать backup старых HTML файлов ✅ **(Создан backup_20251015)**
- [x] ✅ **РАЗБЛОКИРОВАНО:** Миграция превысила порог 80% (текущий прогресс: 36/42 = 85.7%)

**Статус разблокировки:** Старые HTML шаблоны (DocEdit.html, DocEditAct.html, DocEditPassport.html) и legacy JavaScript код все еще используются 6 немигрированными библиотеками (Passport, KMH_MI2816, 2 Common библиотеки и 2 несуществующие). Можно начинать осторожное удаление legacy функций, не затрагивая оставшиеся библиотеки.
...
- Остались: Passport (отложен), KMH_MI2816 (специальный случай), Common* (не существуют)
...
**Последнее обновление:** 2025-10-15 (вечер)
