...
TN_Doc v1.4.3 объединяет backend на ASP.NET Core 8.0 и четыре frontend пакета на Vue 3:

- **Backend**: .NET 8.0 приложение с FastReport и бизнес-логикой
- **Frontend**: StatusBar, Configurator, Document Editor, Shared (npm workspaces)
- **Submodules**: tn.docgeneral (48 библиотек документов), tn_toolsfastreport, winprutil
- **Интеграция**: Vue компоненты публикуются в `wwwroot/{statusbar|configurator|document-editor}/`

```mermaid
flowchart TB
    Start([Начало]) --> Submodules[git submodule update --init --recursive]
    Submodules --> Clean[dotnet clean]
    Clean --> Restore[dotnet restore]
    Restore --> NpmInstall[npm install - TN_Doc/Client]
    NpmInstall --> BuildBackend[Сборка Backend - dotnet build]
    BuildBackend --> BuildVue[Сборка Vue - npm run build:all]
    BuildVue --> Tests[Запуск тестов - dotnet test]
    Tests --> Package[Создание пакета - dotnet publish]
    Package --> End([Завершение])

    style Submodules fill:#ff6b6b
    style BuildVue fill:#42b883
    style BuildBackend fill:#512bd4
```

**Обязательные требования:**
- .NET SDK 10.0+ (для разработки) или .NET Runtime 8.0.13+ (для запуска)
...
# 3. Установить npm зависимости для всех Vue компонентов (первый раз)
cd TN_Doc/Client
npm install
cd ../..
...
# 5. Собрать все Vue компоненты (StatusBar, Configurator, Document Editor)
cd TN_Doc/Client
npm run build:all
cd ../..
...
TN_Doc использует **npm workspaces** для управления четырьмя пакетами Vue 3:

| Компонент | Версия | Порт dev | Назначение |
|-----------|--------|----------|------------|
| **statusbar** | 1.4.2 | 5173 | Мониторинг состояния системы в реальном времени (SignalR) |
| **configurator** | 1.4.2 | 5174 | Веб-интерфейс для управления конфигурацией приложения |
| **document-editor** | 1.4.2 | 5175 | Редактор документов с поддержкой истории изменений |
| **shared** | - | - | Общие утилиты (logger.ts, типы) |

#### Технологический стек Vue компонентов

- **Framework**: Vue 3.4.21 + TypeScript 5.4.5 + Vite 5.2.0
- **UI Library**: PrimeVue 4.2.6 + PrimeIcons 7.0.0
- **State Management**: Pinia 2.1.7
- **HTTP Client**: Axios 1.6.7
- **Real-time**: SignalR 8.0.5 (StatusBar)
- **Router**: Vue Router 4.2.5 (Document Editor)
...
# Устанавливает зависимости для всех workspaces одной командой
npm install

# Структура node_modules:
# - TN_Doc/Client/node_modules/ - общие зависимости (TypeScript, @types/node)
# - statusbar/node_modules/ - специфичные для StatusBar (Vite, Vue, PrimeVue)
# - configurator/node_modules/ - специфичные для Configurator
# - document-editor/node_modules/ - специфичные для Document Editor
# - shared/node_modules/ - утилиты и типы (logger.ts, types/)
```
...
# Примечание: По умолчанию порты:
# - StatusBar: 5173
# - Configurator: 5174
# - Document Editor: 5175
# Можно изменить в соответствующем vite.config.ts
```
...
# TypeScript type checking для всех workspaces
npm run type-check

# Или проверить конкретный workspace
...
package-deb:
  ...
  script:
    ...
    - test -d ./publish/linux/wwwroot/document-editor || exit 1
...
**Ошибка: "Project file does not exist" для tn.docgeneral/TN_Tools**

```bash
# Проверить и инициализировать submodules
...
| Vue компоненты (statusbar + configurator + document-editor) | ~5-10 MB |
| FastReport шаблоны (.frx) | ~20 MB |
| Конфигурационные файлы (Cfg/*.json) | ~5 MB |
| Submodules (tn.docgeneral - 48 DLLs, tn_toolsfastreport) | ~50 MB |
| **Итого** | **~280-295 MB** |
...
- [ ] Git submodules обновлены: `git submodule update --remote`
...
ls -la TN_Doc/bin/Release/net8.0/publish/wwwroot/{statusbar,configurator,document-editor}/
```
...
# Убедитесь что package.json использует workspaces
cat package.json | grep workspaces
# Должно показать: "workspaces": ["statusbar", "configurator", "document-editor", "shared"]
```
