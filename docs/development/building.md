...
| `Configurator открывается пустым / 404 на configurator.*.js` | Потерян `wwwroot/configurator/.vite/manifest.json` в publish или CI artifacts | Проверить, что `.vite/` копируется как hidden directory (csproj + CI job artifacts) |
