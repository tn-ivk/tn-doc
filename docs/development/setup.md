...
| .NET SDK | 10.0 или выше (для разработки) |
...
**Важно:** Для разработки требуется .NET SDK 10.0+, но приложение компилируется под .NET 8.0 Runtime.
...
- **statusbar** - мониторинг состояния системы в реальном времени (SignalR)
...
```mermaid
graph TB
    subgraph "Client Workspaces (npm workspaces)"
        Root[Client/ - корневой package.json<br/>TypeScript 5.4.5, @types/node]
        StatusBar[statusbar/ - v1.4.2<br/>Vue 3.4.21 + SignalR 8.0.5]
        Configurator[configurator/ - v1.4.2<br/>Vue 3.4.21 + Axios 1.6.7]
        Editor[document-editor/ - v1.4.2<br/>Vue 3.4.21 + Router 4.2.5]
        Shared[shared/<br/>logger.ts, types/]
    end
    ...
    StatusBar --> SB_Deps[Vite 5.2.0<br/>PrimeVue 4.2.6<br/>Pinia 2.1.7]
    Configurator --> CF_Deps[Vite 5.2.0<br/>PrimeVue 4.2.6<br/>Pinia 2.1.7]
    Editor --> ED_Deps[Vite 5.2.0<br/>PrimeVue 4.2.6<br/>Pinia 2.1.7]
```

### Общая инфраструктура (shared/)

Пакет `shared/` содержит общую инфраструктуру для всех Vue компонентов:
...
- Общие типы данных для типобезопасности
...
# 2. Проверить статус submodules
git submodule status
# Должно показать: tn.docgeneral, tn_toolsfastreport, winprutil (Windows)

# 3. Восстановить зависимости .NET
dotnet restore
...
# 6. Установить npm зависимости для всех workspaces
cd TN_Doc/Client
npm install

# 7. Собрать все Vue компоненты
npm run build:all
cd ../..

# 8. Проверить наличие собранных артефактов
ls -la TN_Doc/wwwroot/statusbar/assets
ls -la TN_Doc/wwwroot/configurator/assets
ls -la TN_Doc/wwwroot/document-editor/assets
...
- [ ] .NET SDK 10.0+ установлен и проверен (`dotnet --version`)
...
- [ ] Git submodules инициализированы (`git submodule update --init --recursive`)
...
### Ошибка: "Could not find a part of the path 'tn.docgeneral'" или проект не компилируется

Причина: Git submodules не инициализированы.
