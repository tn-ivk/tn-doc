...
- При изменении `CfgApp.json` обновляйте `docs/configs/passport_devices_useElis.json` вручную (скрипт генерации в репозитории не хранится).
...
### Поле `Edit` для AdditionalInfo (СИКН №)
**Изменение:** в актуальных конфигах `CfgEditPassport*.json` для поля `SIKN_Number` установлен флаг `"Edit": false`.

**Что это даёт:**
- Поле "СИКН №" отображается в форме, но недоступно для ручного изменения оператором.
- Значение остаётся частью документа и приходит из исходных данных/бэкенда.
- Правило применено для ГОСТ Р 50.2.040, МИ 3532, ЕАЭС и ForActNP конфигураций.

### Валидация дробной части (`RoundValue`)
**Актуальная логика:** при проверке количества знаков после запятой хвостовые нули игнорируются.

**Примеры:**
- `1,10` при `RoundValue = 1` — валидно (значащая дробная часть `1`).
- `1,200` при `RoundValue = 1` — валидно (значащая дробная часть `2`).
- `1,2340` при `RoundValue = 2` — невалидно (значащая дробная часть `234`).

Фронтенд использует общую проверку `isWithinFractionDigits()` в компоненте ввода и store, чтобы исключить расхождения между UI-валидацией и сохранением.
