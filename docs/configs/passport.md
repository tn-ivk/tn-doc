...
- При **ручном выборе** метода для ведущего параметра → метод автоматически копируется в ведомый
- При **заполнении из ELIS** → метод ведущего параметра также копируется в ведомый (v1.4.4+)
...
**Поведение при заполнении из ELIS (v1.4.4+):**
- Метод испытаний из ELIS записывается для ведущего параметра (leader)
- Автоматически синхронизируется в ведомый параметр (follower)
- Оба параметра помечаются флагом `elisFilled = true`
- История изменений фиксирует `trackElisLoad` для обоих параметров
