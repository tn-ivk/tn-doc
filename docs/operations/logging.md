...
**Режим разработки (Development):** логи пишутся в папку `logs` рядом с исполняемым файлом.  
Пример:
```
TN_Doc\bin\Debug\net8.0\logs\
```

**Production (служба/установка):**
```
...
**Структура файлов логов (по умолчанию):**
```
logs/
...
- [ ] Указана версия приложения (из `TN_Doc.csproj` или заголовка главной страницы)
