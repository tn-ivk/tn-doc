...
- **Document Editor** — редактор паспортов качества в браузере (Vue 3 + PrimeVue, в разработке)
...
### Document Editor (v1.4.4+, в разработке)
- **Назначение:** Редактирование паспортов качества в браузере без FastReport
- **Технологии:** Vue 3 + TypeScript + PrimeVue
- **Основные компоненты:**
  - `DocumentPassportEditor` — главный редактор паспорта
  - `PassportAdditionalInfo` — поля AdditionalInfo с историей изменений
  - `PassportQualityParameters` — таблица параметров качества
  - `FormFieldWithHistory` — поля с индикатором истории
  - Компоненты истории: `FieldHistoryIndicator`, `FieldHistoryPopup`
- **Функции:**
  ...
  - Интеграция с OPC (чтение данных с устройств)
  - История изменений полей (отслеживание источников: ELIS/Manual/IVK)
  - Автозаполнение зависимых параметров
  - Валидация диапазонов значений
- **Статус:** Активная разработка на ветке `developWork`, готовность к production
...
- **Курсор:** pointer (указывает на интерактивность)
...
**Триггер:** Наведение курсора (hover) открывает FieldHistoryPopup с детальной историей изменений
