...
- Белый фон (#ffffff), чёрный текст (#212121)
- Отступы: 0.4rem 0.75rem
...
| Источник | Текст подсказки |
|----------|-----------------|
| ELIS | "Из протокола ЕЛИС" |
| ReturnToELIS | "Из протокола ЕЛИС" |
| Manual | "Изменено вручную" |
| IVK | "Рассчитано ИВК" |
| ElisMissing | "Нет в протоколе ЕЛИС" |
| DefaultMethod | "Метод по умолчанию" |
| Unknown | "Источник неизвестен" |

**Поведение:** Индикатор служит только для визуальной информации о источнике данных. При наведении показывается всплывающая подсказка. Клик по индикатору не выполняет никаких действий.
...
**v1.4.4 (2025-12-09)** - Всплывающие подсказки для индикаторов истории
- ✅ **Добавлены tooltip'ы для индикаторов источника данных:**
  - При наведении на индикатор показывается краткое описание источника
  - Используется директива `v-tooltip` из PrimeVue
  - Белый фон подсказки с чёрным текстом, отступы для читаемости
  - Тексты подсказок: "Из протокола ЕЛИС", "Изменено вручную", "Рассчитано ИВК", "Нет в протоколе ЕЛИС", "Метод по умолчанию"
