...
История изменений работает **только при включенном ELIS**:
...
Флаг может быть задан глобально или на уровне устройства (`Devices[].Elis.Use`). Если ELIS выключен, история не сохраняется и не возвращается на фронтенд.
