...
### Координация popup-ов (v1.4.4+)

**Проблема:** Каждый компонент `*WithHistory` имеет свой экземпляр `FieldHistoryPopup`. При последовательных кликах на разные индикаторы открывалось несколько popup-ов одновременно.

**Решение:** EventBus для координации закрытия popup-ов.

**Файл:** `TN_Doc/Client/document-editor/src/utils/historyPopupEvents.ts`

```typescript
// Подписка на событие закрытия (вызывается в FieldHistoryPopup.onMounted)
const unsubscribe = onCloseAllHistoryPopups(() => {
  overlayPanel.value?.hide();
});

// Закрытие всех popup-ов перед открытием нового
closeAllHistoryPopups();
historyPopup.value?.show(event);
```

**Принцип работы:**
1. `FieldHistoryPopup` при монтировании подписывается на событие закрытия
2. При клике на индикатор вызывается `closeAllHistoryPopups()` → все popup-ы закрываются
3. Затем открывается новый popup
4. При размонтировании компонент отписывается от события

**Результат:** Одновременно может быть открыто только одно окно истории изменений.
...
**v1.4.4 (2025-11-27)** - Исправление бага с множественными окнами истории
- ✅ **Исправлен баг с множественными popup-ами истории:**
  - При последовательных кликах на индикаторы истории открывалось несколько окон одновременно
  - Добавлен EventBus (`historyPopupEvents.ts`) для координации закрытия popup-ов
  - `FieldHistoryPopup` подписывается на событие закрытия при монтировании
  - Все компоненты `*WithHistory` закрывают другие popup-ы перед открытием нового
  - Теперь одновременно может быть открыто только одно окно истории
