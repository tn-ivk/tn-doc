...
graph TB
    ...
    Store --> ApiService
    ApiService <-->|HTTP REST| Backend API

    Note over Backend API: API endpoints для работы<br/>с файлами конфигурации<br/>(через общие контроллеры)
```
...
- [StatusBar Architecture](./statusbar.md) - планируемая функциональность
