...
Document Editor — это веб-приложение для редактирования документов непосредственно в браузере, построенное на **Vue 3 + TypeScript + PrimeVue + Vue Router**. Компонент предоставляет продвинутый UI для редактирования паспортов качества, актов и других документов с поддержкой интеграции с ELIS, OPC, системой истории изменений и автозаполнением зависимых полей.

**Статус**: В активной разработке (developWork branch)
...
- ✅ **Редактирование паспортов качества** - продвинутая форма с таблицей параметров
- ✅ **Интеграция с ELIS** - загрузка лабораторных данных из протоколов
- ✅ **Интеграция с OPC** - прямое чтение данных с измерительных приборов
- ✅ **Система истории изменений полей** - отслеживание источника данных (требует включенного ELIS)
- ✅ **Автозаполнение зависимых параметров** - автоматический расчёт связанных полей
- ✅ **Валидация в реальном времени** - проверка обязательных полей, формата, округления

### Планируется в v1.4.5+
- 🔄 **Редактирование актов** - базовая форма редактирования
- 🔄 **Редактирование других типов документов** - общая форма для Report, Journal и др.
...
# Выходные файлы: TN_Doc/wwwroot/dist/document-editor/
```
...
## Known Limitations (v1.4.4)

- ⚠️ **История изменений требует включенного ELIS** (`IsUsedElis = true` в CfgApp.json)
- ⚠️ **ФИО пользователя недоступно** - используется "Пользователь" вместо реального имени
- ⚠️ **Реализовано только для Passport** - другие типы документов в разработке
- ⚠️ **Офлайн режим не поддерживается** - требуется постоянное подключение к серверу
- ⚠️ **Нет мобильной версии** - UI оптимизирован только для desktop

## Roadmap

### v1.4.5 (Q1 2025)
- [ ] Редактор актов (DocumentActEditor) - production ready
- [ ] Общий редактор для Report/Journal
- [ ] Улучшенная валидация с подсказками
...
- [ ] Мобильная версия
- [ ] Расширенная интеграция с OPC UA
...
- [ELIS Integration](../integration/elis.md)
