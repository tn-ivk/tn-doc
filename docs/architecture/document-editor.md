...
| № | Наименование показателя | Метод испытаний | Документы* | Измерение | Предпросмотр |
|---|------------------------|----------------|-----------|-----------|--------------|
...
- ✅ **Встроенные иконки редактирования**:
  - ✏️ в колонке "Предпросмотр" - открывает `ResultEditDialog`
...
**Использование:**
- Открывается при клике на иконку редактирования в колонке "Предпросмотр"
...
- Ввод граничного значения (опционально, скрыто при загрузке из ELIS)
- Ввод текстового представления результата (например, "Менее 4,0", скрыто при загрузке из ELIS)
- Сброс метода к стандартному значению (кнопка видна только при активном limitValueActivate)
...
interface Props {
  visible: boolean;              // Видимость диалога
  parameterName?: string;        // Название параметра
  isElisEnabled?: boolean;       // Флаг загрузки из ELIS (скрывает поля предельного значения)
  initialMethod?: MethodOption;  // Исходный метод для инициализации формы
}
```
...
interface ManualMethodPayload {
  ...
  limitValueActivate: boolean;   // Флаг использования предельного значения
  limitValue?: number;           // Граничное значение (опционально)
  limitValueString?: string;     // Текстовое представление результата
  isDefault: boolean;            // Сделать методом по умолчанию
}
```
...
|---------|-------------------|
| Чекбокс "Использовать предельное значение" | `!isElisEnabled` |
| Поле "Предельное значение" | `!isElisEnabled` |
| Поле "Строка результата при превышении" | `!isElisEnabled` |
| Кнопка "Сброс" | `form.limitValueActivate` |
...
- При загрузке данных из ELIS (`isElisEnabled = true`) поля предельного значения скрыты
- Кнопка "Сброс" видна только когда активен чекбокс "Использовать предельное значение"
...
**2025-11-28 - Улучшение диалога редактирования метода испытаний**
- ✅ **Изменена логика видимости полей предельного значения:**
  - Поля "Предельное значение" и "Строка результата при превышении" теперь скрыты при `isElisEnabled = true`
  - Ранее поля зависели от `form.limitValueActivate`
- ✅ **Кнопка "Сброс" теперь условно видима:**
  - Отображается только когда `form.limitValueActivate = true`
  - Позволяет сбросить метод к стандартному значению
- 📝 Компоненты: `ManualMethodDialog.vue`
- 📝 Обновлена документация с таблицей логики видимости полей

**2025-11-27 - Сохранение флага ELIS при редактировании метода**
- ✅ **Исправлено сохранение флага ELIS при редактировании метода испытаний:**
  - Флаг ELIS метода сохраняется если название метода не изменялось
  - Флаг ELIS результата сохраняется если значение результата не изменялось
...
- ✅ **Мгновенное обновление предупреждения "отсутствует в справочнике":**
  ...
  - Добавлен callback `addMethodToLocalDictionary` для обновления локального списка методов в схеме
  - Не требуется перезагрузка страницы для скрытия предупреждения
- 📝 Компоненты: `usePassportEditor.ts`, `PassportQualityTable.vue`, `DocumentPassportEditor.vue`
- 📝 Логика проверки изменения названия метода в `handleMethodUpdate()`
