...
> **⚠️ ДОКУМЕНТ УСТАРЕЛ**: Этот документ описывает компонент StatusBar, который НЕ реализован в текущей версии проекта (1.3.8).
>
> **Текущий статус**:
> - Директория `TN_Doc/Client/statusbar/` отсутствует в структуре проекта
> - Build output `TN_Doc/wwwroot/statusbar/` не создается
> - SignalR Hubs для real-time мониторинга не используются
> - Backend сервисы мониторинга (StatusMonitoringService, StatusProvider) не реализованы
>
> Документация сохранена как справочный материал для возможной будущей реализации функциональности мониторинга состояния системы.

## Обзор (исторический)

StatusBar — это планировавшийся real-time компонент мониторинга здоровья системы, который должен был быть построен на **Vue 3 + TypeScript + PrimeVue** с использованием **SignalR** для двусторонней связи.

**Статус**: Не реализован
**Планировавшиеся возможности**:
