...
Модули документов динамически загружаются напрямую в контроллерах через reflection для создания экземпляров `DocGeneral` из отдельных DLL библиотек из субмодуля `tn.docgeneral`.
...
sequenceDiagram
    ...
    Controller->>Controller: Определить путь к DLL по IdDevice и IdDoc
    Controller->>FileSystem: Проверка существования DLL
    FileSystem-->>Controller: FileInfo

    alt DLL файл существует
        Controller->>Assembly: LoadFrom(dllPath)
        Assembly-->>Controller: Загруженная сборка
        Controller->>Assembly: GetTypes() где BaseType == DocGeneral
        Assembly-->>Controller: Type информация
        Controller->>Assembly: CreateInstance(type, args)
        Assembly->>DocGeneral: new DocModule(options, idDevice, idDoc, baseDirectory)
        DocGeneral-->>Controller: Экземпляр модуля
    else DLL не найдена
        Controller-->>Controller: Логирование ошибки и возврат 500
    end
```
...
```csharp
// HomeController.cs - загрузка модуля для просмотра
var dllPath = GetDocDllPath(IdDevice, IdDoc); // Определение пути к DLL
var assembly = Assembly.LoadFrom(dllPath);
var docType = assembly.GetTypes()
    .Single(x => x.BaseType?.Name == nameof(DocGeneral));

var doc = (DocGeneral)assembly.CreateInstance(
    docType.FullName,
    false,
    BindingFlags.Default,
    null,
    new object[] { _options, IdDevice, IdDoc, AppContext.BaseDirectory },
    null,
    null);
...
Путь к каждому модулю обычно определяется в `CfgApp.json` через конфигурацию устройства и документа. Модули документов располагаются в субмодуле `tn.docgeneral`:
...
- Конфигурационные файлы документов (`Cfg*.json`, `CfgEdit*.json`) загружаются из файловой системы при каждом обращении
...
**Динамическая загрузка через Reflection в контроллерах:**
```csharp
// Загрузка модуля документа в контроллере (HomeController/DirEditorController)
public DocGeneral LoadDocModule(int idDevice, IdDoc idDoc)
{
    var dllPath = GetDocDllPath(idDevice, idDoc); // Определение пути из конфигурации
    var pathToDll = Path.Combine(AppContext.BaseDirectory, dllPath.TrimStart('/', '\\'));
...
var docInstance = LoadDocModule(idDevice, idDoc);
...
**Примечание**: В текущей реализации отсутствуют выделенные сервисы для обработки обновлений. Вся логика парсинга, валидации и сохранения находится непосредственно в методах модуля документа (например, `DocPassport.Update.cs`).
