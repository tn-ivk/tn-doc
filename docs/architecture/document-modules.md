...
- **Особенности**:
  ...
  - Для `Act_N_GOSTR50.2.040(G)(Export_Rus).frx` реализован посменный вывод массы нетто прописью:
    - при разных обозначениях нефти по ГОСТ Р 51858 для смен выводится чередование блоков "масса прописью + обозначение ГОСТ" для каждой смены;
    - при одном обозначении для всего акта сохраняется общий вывод массы из `Common.ValueInWords.WholePart`.
