...
- **Маршрутизация**: `{controller=Home}/{action=Index}/{id?}`

## Base URL

```
Kestrel (dotnet run): http://localhost:5000  (https://localhost:5001)
IIS Express (VS):     http://localhost:38509
Production:           зависит от хоста/прокси и настроек запуска
```
...
`IdDoc` — числовое значение enum `TN.DocData.IdDoc` (биндинг регистронезависим).
...
Возвращает строку/путь для редактирования, формируемый модулем документа (`DocGeneral.GetEditDoc`). В текущей версии это основной механизм редактирования.
...
Сохраняет документ через механизм `DocGeneral.SaveDoc()` (JSON‑данные передаются строкой).
...
**Ответ:** JSON‑строка со словарями, включая пользователей
...
**Ответ:** JSON‑строка с массивом недопустимых символов
