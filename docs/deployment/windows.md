...
При обновлении через MSI автоматически:
1. Останавливается текущая служба
2. Создаётся бэкап в `C:\ProgramData\TN_Doc\backups\` (PowerShell скрипт `Backup.ps1`)
3. Удаляется старая версия (MajorUpgrade)
4. Устанавливается новая версия
5. Запускается служба
