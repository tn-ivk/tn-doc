<template>
  <Dialog
    :visible="visible"
    modal
    class="act-manual-signer-dialog"
    header="Ручной ввод"
    :closable="false"
...
export interface ActManualSignerPayload {
  name: string;
}
...
const emit = defineEmits<{
  ...
  confirm: [payload: ActManualSignerPayload];
}>();
...
function handleConfirm() {
  ...
  const payload: ActManualSignerPayload = {
    name: form.name.trim()
  };
...
<style scoped>
.act-manual-signer-dialog :global(.p-dialog) {
  padding: 10px;
}

.act-manual-signer-dialog :global(.p-dialog-header) {
  text-align: center;
  justify-content: center;
}

.act-manual-signer-dialog :global(.p-dialog-content) {
  padding: 1.5rem;
}
