...
/// <summary>
/// Параметризованные тесты для библиотек контроля плотности КМХ (PP).
///
/// Покрывает два модуля документов контроля преобразователя плотности:
/// - KMH_PP: Контроль ПП - преобразователь плотности
/// - KMH_PP_Areom: Контроль ПП ареометром - преобразователь плотности по ареометру
///
/// Приоритет: Фаза 2 - KMH Documents
/// </summary>
[TestFixture]
public class KmhDensityTests
{
    ...
    {
        _testBasePath = Path.Combine(Path.GetTempPath(), $"KmhDensityTests_{Guid.NewGuid()}");
    ...
    [TestCase(IdDoc.KMH_PP, typeof(KMH_PP))]
    [TestCase(IdDoc.KMH_PP_Areom, typeof(DocDocKMH_PP_Areom))]
    ...
    private DocGeneral CreateDocumentInstance(IdDoc idDoc, DbContextOptions<DocGeneral> dbOptions = null)
    {
        ...
        return idDoc switch
        {
            ...
            IdDoc.KMH_PP_Areom => new DocDocKMH_PP_Areom(
                dbOptions,
                _mockAppConfig.Object,
                idDevice: 1,
