...
/// <summary>
/// Параметризованные тесты для библиотек контроля параметров КМХ (PV, PW).
///
/// Покрывает два модуля документов контроля параметров:
/// - KMH_PV: Контроль ПВ - преобразователь объема
/// - KMH_PW: Контроль ПВ - преобразователь массы
///
/// Приоритет: Фаза 2 - KMH Documents
/// </summary>
[TestFixture]
public class KmhParameterTests
{
    ...
    {
        _testBasePath = Path.Combine(Path.GetTempPath(), $"KmhParameterTests_{Guid.NewGuid()}");
    ...
    [TestCase(IdDoc.KMH_PV, typeof(KMH_PV))]
    [TestCase(IdDoc.KMH_PW, typeof(DocKMH_PW))]
