...
/// <summary>
/// Параметризованные тесты для библиотек контроля потока КМХ (PR).
///
/// Покрывает два модуля документов контроля преобразователя расхода:
/// - KMH_PR_PR: Контроль ПР по ПР (ТПР) - преобразователь расхода по контрольному ПР
/// - KMH_PR_PU: Контроль рабочего ПР по ПУ - преобразователь расхода по поверочной установке
///
/// Приоритет: Фаза 2 - KMH Documents
/// </summary>
[TestFixture]
public class KmhFlowTests
{
    ...
    {
        _testBasePath = Path.Combine(Path.GetTempPath(), $"KmhFlowTests_{Guid.NewGuid()}");
    ...
    [TestCase(IdDoc.KMH_PR_PR, typeof(KMH_PR_PR))]
    [TestCase(IdDoc.KMH_PR_PU, typeof(KMH_PR_PU))]
    ...
    [TestCase(IdDoc.KMH_PR_PR)]
    [TestCase(IdDoc.KMH_PR_PU)]
